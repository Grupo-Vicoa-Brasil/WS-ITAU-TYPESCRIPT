...
export default function ITAU2 (req, res) {
  app.get('/cpfs', (req: Request, res: Response) => {
    connection.query('SELECT distinct(cpf), entidade, estado FROM ws_itauInput8 where estado is null limit 1', function (err: Error, result: Response, fields) {
  ...
  const trabalharCpf = async (cpf: string, entidade: string) => {
    ...
    const { data } = await axios.post(`http://refin.datafast.com.br/itaubmg?usuario=FINANSERV&senha=Fw23asWs2efyf3&usuarioBanco=${process.env.login_itau8}&senhaBanco=${process.env.senha_itau8}&cpf=${cpf}&convenioRefin=${entidade}&dadosCadastrais=SIM`)
    ...
    if (resultado.RESULTADO.ERRO === 'NAO') {
      const query = `UPDATE ws_itauInput8 set estado = '1' where estado IS NULL and cpf = '${cpf}' and entidade = '${entidade}'  `
      // eslint-disable-next-line node/handle-callback-err
      connection.query(query, function (err: Error, result, fields) {
        console.log('ITAU 9 - CPF Consultado Atualizado!')
        console.log(new AppUpdated('🌟 ITAU 9 - CPF Constultado Atualizado!'))
      })
  ...
  return res.send({ message: 'Server 9 - ITAU', status: 'Online', file: 'itau9.routes.ts' })
}
