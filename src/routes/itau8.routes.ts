...
export default function ITAU2 (req, res) {
  app.get('/cpfs', (req: Request, res: Response) => {
    connection.query('SELECT distinct(cpf), entidade, estado FROM ws_itauInput7 where estado is null limit 1', function (err: Error, result: Response, fields) {
  ...
  const trabalharCpf = async (cpf: string, entidade: string) => {
    ...
    const { data } = await axios.post(`http://refin.datafast.com.br/itaubmg?usuario=FINANSERV&senha=Fw23asWs2efyf3&usuarioBanco=${process.env.login_itau7}&senhaBanco=${process.env.senha_itau7}&cpf=${cpf}&convenioRefin=${entidade}&dadosCadastrais=SIM`)
  ...
  return res.send({ message: 'Server 8 - ITAU', status: 'Online', file: 'itau8.routes.ts' })
}
