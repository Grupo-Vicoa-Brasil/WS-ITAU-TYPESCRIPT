...
export default function ITAU2 (req, res) {
  ...
  const trabalharCpf = async (cpf: string, entidade: string) => {
    ...
    const { data } = await axios.post(`http://refin.datafast.com.br/itaubmg?usuario=FINANSERV&senha=Fw23asWs2efyf3&usuarioBanco=${process.env.login_itau4}&senhaBanco=${process.env.senha_itau4}&cpf=${cpf}&convenioRefin=${entidade}&dadosCadastrais=SIM`)
    ...
    if (resultado.RESULTADO.ERRO === 'NAO') {
      ...
      connection.query(query, function (err: Error, result, fields) {
        console.log('ITAU 5 - CPF Consultado Atualizado!')
        console.log(new AppUpdated('🌟 ITAU 5 - CPF Constultado Atualizado!'))
      })
  ...
  return res.send({ message: 'Server 4 - ITAU', status: 'Online', file: 'itau5.routes.ts' })
}
